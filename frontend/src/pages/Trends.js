  ...
  const series = useMemo(() => {
    ...
    const weekly = sorted.map((m) => {
      ...
      return {
        ...
        outstanding: Math.round(k.totalOutstanding),
        d90: Math.round(k.d90), d60: Math.round(k.d60), d30: Math.round(k.d30),
        target: Math.round(k.totalNewTarget), collPct: Number(k.avgCollPct.toFixed(1)),
      };
    });
    if (granularity === "weekly") return weekly;
    // monthly aggregation (average within month)
    const groups = {};
    weekly.forEach((w) => {
      const g = groups[w.monthKey] || (groups[w.monthKey] = { label: w.monthLabel, items: [] });
      g.items.push(w);
    });
    return Object.keys(groups).sort().map((key) => {
      ...
      return {
        label: g.label,
        outstanding: avg("outstanding"), d90: avg("d90"), d60: avg("d60"), d30: avg("d30"),
        target: avg("target"), collPct: Number((g.items.reduce((s, x) => s + x.collPct, 0) / g.items.length).toFixed(1)),
      };
    });
    ...
    <div className="space-y-8" data-testid="trends-page">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Track how outstanding, aging and collection efficiency move across meetings.</p>
        </div>
        ...
        <div className="h-[320px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={series} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              <defs>
                {[["d30", "#2563EB"], ["d60", "#F59E0B"], ["d90", "#DC2626"]].map(([k, c]) => (
              ...
              <Tooltip content={({ active, payload, label }) => curTip(active, payload, label)} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6 shadow-none" data-testid="trend-collection">
          <h3 className="text-base font-medium mb-1">Average Collection Efficiency</h3>
          <p className="text-xs text-muted-foreground mb-5">Mean collection % across reps</p>
        ...
        <Card className="p-6 shadow-none" data-testid="trend-target">
          <h3 className="text-base font-medium mb-1">Outstanding vs Collection Target</h3>
          ...
          <div className="h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={series} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="target" name="Target" stroke="#2563EB" strokeWidth={2} strokeDasharray="5 4" dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      {series.length < 2 && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <TrendingUp className="h-4 w-4" /> Add more weekly meetings to see richer trends.
        </div>
      )}
    </div>
  );
