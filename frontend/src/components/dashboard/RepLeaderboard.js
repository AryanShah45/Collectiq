...
function pctColor(p) {
  if (p >= 25) return "text-[#16A34A]";
  if (p >= 15) return "text-[#F59E0B]";
  return "text-[#DC2626]";
}
...
export default function RepLeaderboard({ meeting, company }) {
  ...
  return (
    <Card className="p-6 shadow-none" data-testid="rep-leaderboard">
      <h3 className="text-base font-medium mb-1">Sales Rep Leaderboard</h3>
      <p className="text-xs text-muted-foreground mb-5">Ranked by collection efficiency</p>
      <div className="space-y-2">
        {rows.map((r, i) => (
          <div
            key={r.name}
            className="grid grid-cols-12 items-center gap-3 px-3 py-3 rounded-md border border-border hover:bg-secondary/50 transition-colors"
          ...
          >
            <div className="col-span-5 sm:col-span-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground font-mono">{formatINR(r.collPerDay)}/day</div>
              </div>
            </div>
            ...
            <div className="col-span-2 sm:col-span-3 text-right">
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">Target (WoW)</div>
              <div className="font-mono tabular-nums text-sm">{formatINR(r.newTarget)}</div>
              <Badge variant="outline" className={`mt-0.5 gap-1 text-[10px] font-mono ${r.targetDelta > 0 ? "text-[#DC2626] border-[#DC2626]/30" : r.targetDelta < 0 ? "text-[#16A34A] border-[#16A34A]/30" : "text-muted-foreground"}`}>
                {r.targetDelta > 0 ? <TrendingUp className="h-3 w-3" /> : r.targetDelta < 0 ? <TrendingDown className="h-3 w-3" /> : <Minus className="h-3 w-3" />}
                {formatINR(Math.abs(r.targetDelta))}
              </Badge>
            </div>
          </div>
        ))}
